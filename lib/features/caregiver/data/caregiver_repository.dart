...
class CaregiverRepository {
  ...
  Future<List<AppUser>> getAssignedSeniors(String caregiverId) async {
    ...
    // Fetch each senior's details
    final seniors = <AppUser>[];
    for (final seniorId in assignedSeniors) {
      ...
      if (seniorDoc.exists) {
        seniors.add(AppUser.fromMap({...seniorDoc.data()!, 'uid': seniorDoc.id}));
      }
    }
  ...
  Stream<List<AppUser>> streamAssignedSeniors(String caregiverId) {
    return _firestore
      ...
      for (final seniorId in assignedSeniors) {
        ...
        if (seniorDoc.exists) {
          seniors
              .add(AppUser.fromMap({...seniorDoc.data()!, 'uid': seniorDoc.id}));
        }
      }
