...
class FamilyHomeScreen extends ConsumerWidget {
  ...
  Widget _buildLinkedSeniorView(BuildContext context, WidgetRef ref, dynamic linkedSenior, dynamic user) {
  ...
  Widget _buildSeniorStatusCard(BuildContext context, WidgetRef ref, dynamic linkedSenior) {
    return Card(
      elevation: 4,
      child: Padding(
          ...
          children: [
            Stack(
              children: [
                CircleAvatar(
                  ...
                  child: Text((linkedSenior.name ?? 'S')[0].toUpperCase(), style: const TextStyle(fontSize: 28, color: Colors.white)),
                ),
                Positioned(
                  bottom: 0,
                  right: 0,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(linkedSenior.name ?? 'Your Senior', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                  const SizedBox(height: 4),
  ...
  Widget _buildQuickActions(BuildContext context, dynamic linkedSenior) {
    return Row(
      children: [
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (email.isNotEmpty) {
                final success = await ref.read(familyLinkControllerProvider.notifier).linkToSenior(email);
                if (ctx.mounted) {
                  Navigator.pop(ctx);
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(success ? 'Linked successfully!' : 'Senior not found'), backgroundColor: success ? Colors.green : Colors.red));
                }
              }
            },
