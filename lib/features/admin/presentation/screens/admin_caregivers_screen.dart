...
class _AdminCaregiversScreenState extends ConsumerState<AdminCaregiversScreen> {
  ...
  Future<void> _approveUser(AppUser caregiver) async {
    ...
    if (confirmed == true && mounted) {
      final success = await ref
          ...
          .approveUser(caregiver.uid);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(success
                ? '${caregiver.name} has been approved'
                : 'Failed to approve ${caregiver.name}'),
  ...
  Future<void> _rejectUser(AppUser caregiver) async {
    ...
    if (confirmed == true && mounted) {
      ...
      final success = await ref
          .read(adminUserControllerProvider.notifier)
          .rejectUser(caregiver.uid, reason);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(success
                ? '${caregiver.name} has been rejected'
                : 'Failed to reject ${caregiver.name}'),
